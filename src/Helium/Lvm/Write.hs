...
module Helium.Lvm.Write (lvmWriteFile, lvmToBytes) where
...
instance Applicative Emit where
  ...
  Couldn't match type ‘State’ with ‘IdMap [(DeclKind, Index)]’
      Expected: Env -> State -> (b, State)
        Actual: State -> (a -> b, State)
    • Possible cause: ‘f’ is applied to too many arguments
      In the first argument of ‘Emit’, namely ‘(f e)’
      In the expression: Emit (f e)
      In an equation for ‘<*>’: (<*>) (Emit f) (Emit e) = Emit (f e)
    |
468 |   (<*>) (Emit f) (Emit e) = Emit ( f e )
  -}
  (<*>) (Emit f) (Emit e) = Emit (\env state -> 
    let (g, state') = f env state  
        (a, state'') = e env state'  
    in (g a, state''))
  pure x = Emit (\_env state -> (x, state))
